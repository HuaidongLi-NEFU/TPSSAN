...
elif FLAGS.dataset_name == "CETUS":
    patch_size = (112, 112, 80)
    FLAGS.root_path = FLAGS.root_path + 'data/CETUS'
    with open(FLAGS.root_path + '/test.txt', 'r') as f:
        image_list = f.readlines()
    image_list = [FLAGS.root_path + "/CETUS/" + item.replace('\n', '')  for item in
                  image_list]
...
def test_calculate_metric():
    net = net_factory(net_type='VNet_4out', in_chns=1, class_num=num_classes, mode="test")
    save_mode_path = os.path.join(snapshot_path, 'VNet_4out_best_model2.pth'.format(FLAGS.model))
