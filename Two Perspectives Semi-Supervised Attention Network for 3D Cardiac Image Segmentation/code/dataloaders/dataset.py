...
class LAHeart(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._base_dir + "/2018LA_Seg_Training Set/" + image_name, 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'label': label}
        if self.transform:
...
class LAHeart_no_read1(Dataset):
    """ LA Dataset """
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        self._base_dir = base_dir
        self.transform = transform
        ...
        # for i in range(len(self.image_list)):
            ...
            # self.image_list_data.append(h5py.File(self._base_dir + "/origin/" + self.image_list[i] , 'r'))
...
class LAHeart_no_read2(Dataset):
    """ LA Dataset """
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        self._base_dir = base_dir
        self.transform = transform
...
class CETUS(Dataset):
    ...
    def __getitem__(self, idx):
        image_name = self.image_list[idx]
        h5f = h5py.File(self._base_dir + "/CETUS/" + image_name, 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'label': label}
        if self.transform:
...
class RandomCrop1(object):
    ...
    def __init__(self, output_size, slice_weight,with_sdf=False):
        self.output_size = output_size
